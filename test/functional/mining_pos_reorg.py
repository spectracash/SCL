...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected SCL and zSCL supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zSCLsupply']:
                assert_equal(node['zSCLsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check SCL and zSCL supply at the beginning
        # ------------------------------------------
        # zSCL supply: 2 coins for each denomination
        ...
        # SCL supply: block rewards minus burned fees for minting
        ...
        # Verify that SCL and zSCL supplies were properly updated after the spends and reorgs
        self.log.info("Check SCL and zSCL supply...")
