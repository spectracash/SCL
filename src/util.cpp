...
const char * const PIVX_CONF_FILENAME = "scl.conf";
const char * const PIVX_PID_FILENAME = "scl.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";


// Spectra only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\scl
// Windows >= Vista: C:\Users\Username\AppData\Roaming\scl
// Mac: ~/Library/Application Support/scl
// Unix: ~/.scl
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Spectra";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\sclParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\sclParams
    // Mac: ~/Library/Application Support/sclParams
    // Unix: ~/.scl-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SpectraParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "SpectraParams";
#else
    // Unix
    return pathRet / ".scl-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty scl.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL){
            std::string strHeader = "# Spectra config file\n"
                          "rpcuser=username\n"
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override scl.conf
