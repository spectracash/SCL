...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter SCL address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no SCL: URI
    if (!uri.isValid() || uri.scheme() != QString(URI_SCLEME))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert scl:// to scl:
    //
    //    Cannot handle this later, because scl:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if (uri.startsWith(URI_SCLEME "://", Qt::CaseInsensitive)) {
        uri.replace(0, std::strlen(URI_SCLEME) + 3, URI_SCLEME ":");
    }
...
{
    QString ret = QString(URI_SCLEME ":%1").arg(info.address);
    int paramCount = 0;
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "scl.lnk";
}
...
{
    return GetAutostartDir() / "scl.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a scl.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Spectra\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the scl app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add scl app to startup item list
