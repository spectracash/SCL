...
CAmount CMasternode::GetMasternodeNodeCollateral(int nHeight) 
{
    ...
    if (nHeight > 600000)	return 20000 * COIN;
    if (nHeight > 350000)	return 15000 * COIN;
    if (nHeight > 200000)	return 10000 * COIN;
    if (nHeight > 1)		return 5000 * COIN;
    return 100000 * COIN;
}

CAmount CMasternode::GetBlockValue(int nHeight)
{
    if (nHeight > 2000000)	return 0.5 * COIN;
    if (nHeight > 1200000)	return 1   * COIN;
    if (nHeight > 800000)	return 2   * COIN;
    if (nHeight > 700000)	return 3   * COIN;
    if (nHeight > 600000)	return 4   * COIN;
    if (nHeight > 500000)	return 5   * COIN;
    if (nHeight > 350000)	return 6   * COIN;
    if (nHeight > 250000)	return 7   * COIN;
    if (nHeight > 200000)	return 6   * COIN;
    if (nHeight > 150000)	return 5   * COIN;
    if (nHeight > 75000)	return 4   * COIN;
    if (nHeight > 30000)	return 3   * COIN;
    if (nHeight > 10000)	return 2   * COIN;
    if (nHeight > 1000)		return 1   * COIN;
    if (nHeight >= 1)		return 200 * COIN;
    return 0 * COIN;
}

CAmount CMasternode::GetMasternodePayment(int nHeight)
{
    return GetBlockValue(nHeight) * 70 / 100;
}

CAmount CMasternode::GetDevPayment(int nHeight)
{
    if (nHeight > 1000) return GetBlockValue(nHeight) * 10 / 100;
    return 0;
}
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 SCL tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 SCL tx -> 1 confirmation
