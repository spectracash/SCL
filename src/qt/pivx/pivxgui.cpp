...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Spectra client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Spectra"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Spectra"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
